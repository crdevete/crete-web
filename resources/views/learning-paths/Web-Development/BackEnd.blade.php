<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Back-End Development</title>
    <link rel="shortcut icon" type="image/png" href="../../../img/CRETE_icon.png" />
    <meta name="description" content="">
    <meta name="author" content="">

    <!-- Bootstrap -->
    <link rel="stylesheet" type="text/css" href="../../css/bootstrap.css">
    <link rel="stylesheet" type="text/css" href="../../fonts/font-awesome/css/font-awesome.css">

    <!-- Stylesheet
    ================================================== -->
    <link rel="stylesheet" type="text/css" href="../../css/style.css">
    <link rel="stylesheet" type="text/css" href="../../css/courses-style.css">
    <link rel="stylesheet" type="text/css" href="../../css/prettyPhoto.css">
    <link href='https://fonts.googleapis.com/css?family=Open+Sans:400,700,800,600,300' rel='stylesheet' type='text/css'>


    <!-- HTML5 shim and Respond.js for IE8 support of HTML5 elements and media queries -->
    <!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
    <!--[if lt IE 9]>
      <script src="https://oss.maxcdn.com/html5shiv/3.7.2/html5shiv.min.js"></script>
      <script src="https://oss.maxcdn.com/respond/1.4.2/respond.min.js"></script>
    <![endif]-->

</head>

<body id="form-body">

  <div class="spinner">
            <div class="rect1"></div>
            <div class="rect2"></div>
            <div class="rect3"></div>
            <div class="rect4"></div>
            <div class="rect5"></div>
     </div>

    <div class="fullscreen_bg">

        <div class="panel">

            <div class="row">

                <div class="col-lg-12">

                    <a href="../../index.html" id="panel-header-form"><img id="form-logo" src="../../../img/CRETE_icon.png"><h4 id="panel-header-text">Crete Agency</h4></a>
                    <h5 id="panel-header-text-small">Back-End Development</h5>


                </div>

                <hr>
            </div>
             <div class="row course-head-note">
					<small id="coursesNote" class="form-text text-muted">NOTE : It is important to take these courses in a row. <i class="fa fa-sort-numeric-asc" aria-hidden="true"></i></small>	        				
				</div>




 <div class="container">
		<div class="row">
			<div class="[ col-xs-12 col-sm-offset-2 col-sm-12 ]">
				<ul class="event-list">
                    <li onclick="window.open('https://www.udemy.com/php-for-complete-beginners-includes-msql-object-oriented')">
						<time>
							<span class="hours">33.5</span>
							<span class="hours-txt">Hours</span>
                            <hr class="courses-line">
							<span class="hours-txt"><i class="fa fa-star" aria-hidden="true"></i></span>
                            <span class="level">Beginner</span>
							
						</time>
						<img alt="Course Website" src="../../../img/Courses/udemy.png" />
						<div class="info">
							
							<div class="row">
								<div class="col-sm-12">
									<h2 class="title">PHP for Beginners - Become a PHP Master - CMS Project</h2>
								</div>
								
							</div>
							<br>
							<div class="row">
								<div class="col-sm-12 course-description">PHP for Beginners: learn everything you need to become a professional PHP developer with practical exercises & projects.</div>
								
							</div>
                        <div class="row">
						        				
						</div>
						</div>
                        
                        </li>


                                           <li onclick="window.open('https://www.udemy.com/the-complete-mysql-developer-course')">
						<time>
							<span class="hours">9.5</span>
							<span class="hours-txt">Hours</span>
                            <hr class="courses-line">
							<span class="hours-txt"><i class="fa fa-star" aria-hidden="true"></i><i class="fa fa-star" aria-hidden="true"></i></span>
                            <span class="level">Intermediate</span>
							
						</time>
						<img alt="Course Website" src="../../../../img/Courses/udemy.png" />
						<div class="info">
							
							<div class="row">
								<div class="col-sm-12">
									<h2 class="title">The Complete MySQL Developer Course</h2>
								</div>
								
							</div>
							<br>
							<div class="row">
								<div class="col-sm-12 course-description">MySQL: Learn how to build Reports and work with Databases for Web Applications using MySQL and PHP</div>
								
							</div>
                        <div class="row">
						        				
						</div>
						</div>
                        
                        </li>


                      
                         <li onclick="window.open('https://www.udemy.com/php-with-laravel-for-beginners-become-a-master-in-laravel')">
						<time>
							<span class="hours">30</span>
							<span class="hours-txt">Hours</span>
                            <hr class="courses-line">
							<span class="hours-txt"><i class="fa fa-star" aria-hidden="true"></i></span>
                            <span class="level">Beginner</span>
							
						</time>
						<img alt="Course Website" src="../../../../img/Courses/udemy.png" />
						<div class="info">
							
							<div class="row">
								<div class="col-sm-12">
									<h2 class="title">PHP with Laravel for beginners - Become a Master in Laravel</h2>
								</div>
								
							</div>
							<br>
							<div class="row">
								<div class="col-sm-12 course-description">Learn to master Laravel to make advanced applications like the real CMS app we build on this course.</div>
								
							</div>
                        <div class="row">
						        				
						</div>
						</div>
                        
                        </li>





                        <li onclick="window.open('https://www.udemy.com/python-web-programming')">
						<time>
							<span class="hours">6</span>
							<span class="hours-txt">Hours</span>
                            <hr class="courses-line">
							<span class="hours-txt"><i class="fa fa-star" aria-hidden="true"></i></span>
                            <span class="level">Beginner</span>
							
						</time>
						<img alt="Course Website" src="../../../../img/Courses/udemy.png" />
						<div class="info">
							
							<div class="row">
								<div class="col-sm-12">
									<h2 class="title">Python Web Programming


</h2>
								</div>
								
							</div>
							<br>
							<div class="row">
								<div class="col-sm-12 course-description">Access and parse the web with Python


</div>
								
							</div>
                        <div class="row">
						        				
						</div>
						</div>
                        
                        </li>







                        <li onclick="window.open('https://www.udemy.com/python-and-django-full-stack-web-developer-bootcamp')">
						<time>
							<span class="hours">32</span>
							<span class="hours-txt">Hours</span>
                            <hr class="courses-line">
							<span class="hours-txt"><i class="fa fa-star" aria-hidden="true"></i></span>
                            <span class="level">Beginner</span>
							
						</time>
						<img alt="Course Website" src="../../../img/Courses/udemy.png" />
						<div class="info">
							
							<div class="row">
								<div class="col-sm-12">
									<h2 class="title">Python and Django Full Stack Web Developer Bootcamp

</h2>
								</div>
								
							</div>
							<br>
							<div class="row">
								<div class="col-sm-12 course-description">Learn to build websites with HTML , CSS , Bootstrap , Javascript , jQuery , Python 3 , and Django 1.11!

</div>
								
							</div>
                        <div class="row">
						        				
						</div>
						</div>
                        
                        </li>









                        <li onclick="window.open('https://www.udemy.com/ruby-programming')">
						<time>
							<span class="hours">5</span>
							<span class="hours-txt">Hours</span>
                            <hr class="courses-line">
							<span class="hours-txt"><i class="fa fa-star" aria-hidden="true"></i></span>
                            <span class="level">Beginner</span>
							
						</time>
						<img alt="Course Website" src="../../../img/Courses/udemy.png" />
						<div class="info">
							
							<div class="row">
								<div class="col-sm-12">
									<h2 class="title">Beginners Ruby Programming Training - No Experience Required

</h2>
								</div>
								
							</div>
							<br>
							<div class="row">
								<div class="col-sm-12 course-description">A Ruby Programming Course Training Video. Learn Real-World Programming Techniques At Your Own Pace.

</div>
								
							</div>
                        <div class="row">
						        				
						</div>
						</div>
                        
                        </li>







                        <li onclick="window.open('https://www.udemy.com/the-complete-ruby-on-rails-developer-course')">
						<time>
							<span class="hours">28.5</span>
							<span class="hours-txt">Hours</span>
                            <hr class="courses-line">
							<span class="hours-txt"><i class="fa fa-star" aria-hidden="true"></i></span>
                            <span class="level">Beginner</span>
							
						</time>
						<img alt="Course Website" src="../../../img/Courses/udemy.png" />
						<div class="info">
							
							<div class="row">
								<div class="col-sm-12">
									<h2 class="title">The Complete Ruby on Rails Developer Course
</h2>
								</div>
								
							</div>
							<br>
							<div class="row">
								<div class="col-sm-12 course-description">Learn to make innovative web apps with Ruby on Rails and unleash your creativity
</div>
								
							</div>
                        <div class="row">
						        				
						</div>
						</div>
                        
                        </li>

               
	
	 <li onclick="window.open('https://www.udemy.com/understand-nodejs')">
						<time>
							<span class="hours">13</span>
							<span class="hours-txt">Hours</span>
                            <hr class="courses-line">
							<span class="hours-txt"><i class="fa fa-star" aria-hidden="true"></i></span>
                            <span class="level">Beginner</span>
							
						</time>
						<img alt="Course Website" src="../../../img/Courses/udemy.png" />
						<div class="info">
							
							<div class="row">
								<div class="col-sm-12">
									<h2 class="title">Learn and Understand NodeJS

</h2>
								</div>
								
							</div>
							<br>
							<div class="row">
								<div class="col-sm-12 course-description">Dive deep under the hood of NodeJS. Learn V8, Express, the MEAN stack, core Javascript concepts, and more.

</div>
								
							</div>
                        <div class="row">
						        				
						</div>
						</div>
                        
                        </li>



                        	 <li onclick="window.open('https://www.udemy.com/the-complete-developers-guide-to-mongodb')">
						<time>
							<span class="hours">13.5</span>
							<span class="hours-txt">Hours</span>
                            <hr class="courses-line">
							<span class="hours-txt"><i class="fa fa-star" aria-hidden="true"></i></span>
                            <span class="level">Beginner</span>
							
						</time>
						<img alt="Course Website" src="../../../img/Courses/udemy.png" />
						<div class="info">
							
							<div class="row">
								<div class="col-sm-12">
									<h2 class="title">The Complete Developers Guide to MongoDB


</h2>
								</div>
								
							</div>
							<br>
							<div class="row">
								<div class="col-sm-12 course-description">Master MongoDB and Mongoose design with a test-driven approach.

</div>
								
							</div>
                        <div class="row">
						        				
						</div>
						</div>
                        
                        </li>





                        	 <li onclick="window.open('https://www.udemy.com/the-complete-nodejs-developer-course-2')">
						<time>
							<span class="hours">26</span>
							<span class="hours-txt">Hours</span>
                            <hr class="courses-line">
							<span class="hours-txt"><i class="fa fa-star" aria-hidden="true"><i class="fa fa-star" aria-hidden="true"></i></i></span>
                            <span class="level">Intermediate</span>
							
						</time>
						<img alt="Course Website" src="../../../img/Courses/udemy.png" />
						<div class="info">
							
							<div class="row">
								<div class="col-sm-12">
									<h2 class="title">The Complete Node.js Developer Course (2nd Edition)


</h2>
								</div>
								
							</div>
							<br>
							<div class="row">
								<div class="col-sm-12 course-description">Learn Node.js by building real-world applications with Node, Express, MongoDB, Mocha, and more!


</div>
								
							</div>
                        <div class="row">
						        				
						</div>
						</div>
                        
                        </li>







				</ul>
                    <div class="row course-facebook">
					<small id="coursesHelp" class="form-text text-muted">NOTE : If you can't afford any of these courses or need it for free, Please ask for free version using course name on our <a style="color: #3B5998" target="_blank" href="https://www.facebook.com/groups/1935149636805516">Facebook Group. <i class="fa fa-facebook-square" aria-hidden="true"></i></a></small>	        				
				</div>
        
			</div>
		</div>
	</div>






        </div>


    </div>

    <!-- jQuery (necessary for Bootstrap's JavaScript plugins) -->
    <script type="text/javascript" src="../../js/jquery.1.11.1.js"></script>
    <!-- Include all compiled plugins (below), or include individual files as needed -->
    <script type="text/javascript" src="../../js/bootstrap.js"></script>


    <!-- Javascripts
    ================================================== -->
    <script type="text/javascript" src="../../js/main.js"></script>
</body>

</html>